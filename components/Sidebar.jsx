    ...
    return (
        <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4  w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}> 
          <div>
            ...
            {/* Modified button with 18px border-radius */}
            <button className={`mt-8 flex items-center justify-center cursor-pointer rounded-[18px] ${expand ? "bg-primary hover:opacity-90 gap-2 p-2.5 w-max" : "group relative h-9 w-9 mx-auto hover:bg-gray-500/30"}`}>
          ...
          <div>
            <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-1g hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"} rounded-[20px]`}>
